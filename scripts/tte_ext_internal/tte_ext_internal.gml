function _tte_ext_input_check_surface() {
...
function _tte_ext_input_handle_mouse() {
	/// Handle mouse

	var mx, my
	mx = (device_mouse_x_to_gui(0) - x - padding_left)
	my = (device_mouse_y_to_gui(0) - y - padding_top)
	...
	if(point_in_rectangle(mx, my, 0, 0, width-1, height-1)) {
	    ...
	    if(mouse_check_button(mb_left)) {
	        ...
	        if(hover_x != mx or hover_y != my && focus) {
	            hover_x = mx
	            hover_y = my
...
function _tte_ext_input_handle_keyboard() {
	/// Handle Keyboard
	if(focus) {
	    ...
	    if(keyboard_string != "" && !ctrl && !alt) {
	        if(tte_ext_has_selection())
				tte_ext_selection_delete()
			if (string_length(tte_ext_input_get_text()) < charLimit) {
...
function _tte_ext_input_handle_caret() {
	/// Handle Caret
	if(focus) {
	    if(forceCaret) {
	        forceCaret = false
...
function tte_ext_selection_delete() {
	...
	if (selStartLine != selEndLine)
		ds_list_delete(lines, selEndLine)
	for (i = selEndLine-1; i > selStartLine; i--)
