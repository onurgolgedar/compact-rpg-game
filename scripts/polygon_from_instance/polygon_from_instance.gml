...
function polygon_from_instance() {
	...
	with(inst) {
		if(rotation == 0) {
			...
			set[1] = [bbox_right, bbox_top]
			set[2] = [bbox_right, bbox_bottom]
			set[3] = [bbox_left, bbox_bottom]*/
			set[0] = [bbox_left - 1, bbox_top - 1]
			set[1] = [bbox_right + 1, bbox_top - 1]
			set[2] = [bbox_right + 1, bbox_bottom + 1]
			set[3] = [bbox_left - 1, bbox_bottom + 1]
		}
		else {
			...
			set[0] = [x - 1,				 y - 1]
			set[1] = [x + sprite_width + 1,  y - 1]
			set[2] = [x + sprite_width + 1,  y + sprite_height + 1]
			set[3] = [x - 1,				 y + sprite_height + 1]
