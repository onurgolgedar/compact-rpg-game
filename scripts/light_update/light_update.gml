...
function light_update(argument0) {
	...
	// Validate argument
	if (__LIGHTING_ERROR_CHECKS && (!ds_exists(light, ds_type_list) or ds_list_size(light) != eLight.Count)) {
	...
	if(culled_shadow_casters != undefined && (dirty or light_last_camera == undefined or !array_equals(light_last_camera, camera))) {
	...
	if(!light_is_directional && global.shadowCastersCullByCollisionMask) {
		...
		ds_list_clear(list)
		collision_rectangle_list(light_area[0], light_area[1], light_area[2], light_area[3], parShadowCaster, false, false, list, false)
