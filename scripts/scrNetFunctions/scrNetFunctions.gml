...
function net_client_send() {
	...
	if (redirection == 65535 and global.clientID != "Local" and global.coopID != "")
		redirection = global.socket_CLIENT
...
function net_client_send_COOP() {
	...
	if (isUDP)
		network_send_udp(global.socket_CLIENT_COOP, global.coopIP, PORT_UDP_COOP, buffer, buffer_tell(buffer))
	else
		network_send_packet(global.socket_CLIENT_COOP, buffer, buffer_tell(buffer))
...
function net_server_send() {
	...
	if (socketID != global.socketID_player) {
		...
		_net_receive_packet(code, data, socketID, 0)
}