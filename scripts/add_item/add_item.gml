function item_add_SERVER(item, accountID, box_i = undefined, box_j = undefined, count = 1) {
...
function item_delete_SERVER(item, accountID, box_i = undefined, box_j = undefined, count = 1) {
	var boxes = global.playerBoxes[? accountID]
	
	var pageScanning = 1
	var sameItem = item_get_exists_SERVER(item, accountID)
	...
	if (!item.isCollectable or sameItem == undefined) {
		if (box_i == undefined) {
		    for (var j = 0; j < global.bc_ver_COMMON; j++) {
		        for (var i = global.bc_hor_COMMON*(pageScanning-1); i < global.bc_hor_COMMON*pageScanning; i++) {
					var box = ds_grid_get(boxes, i, j)
					if (item_get_confirmation_number_COMMON(box.item) == item_get_confirmation_number_COMMON(item)) {
