...
float intensityPoint(vec2 P, vec2 tex, float range) {
	...
	if(!u_AttenuationEnabled) return intensity.a; /**50.0 is for super sharp lightning*/ // ! // ? //! //?
	return intensity.a * attenuation(T, u_AttenuationAlpha, u_AttenuationBeta); /**50.0 is for super sharp lightning*/ // ! // ? //! //?;
}
...
vec4 blendFragment(sampler2D sampler, vec2 coord, float dist) {
	...
	return col0; //(2.5 * col0 + 2.5 * col2 + 6.0 * col4 + 2.5 * col6 + 2.5 * col8) * 0.0625;
}