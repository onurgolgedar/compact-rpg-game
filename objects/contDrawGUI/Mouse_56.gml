...
if (!is_click_blocked() and global.held_box != undefined) {
	if (global.held_from == object_get_name(objSkills_window) and mouseOnSkillBox != undefined) {
		net_client_send(_CODE_SET_SKILLBOX, string(mouseOnSkillBox)+"|"+string(global.held_box.skill.index)+"|-1", BUFFER_TYPE_STRING)
		done = true
	}
	else if (global.held_from == object_get_name(contDrawGUI) and mouseOnSkillBox != undefined) {
