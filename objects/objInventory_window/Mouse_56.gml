	...
	if (is_mouse_on()) {
		...
		if (mouseOn_box != undefined) {
			if (global.held_from_assetName == object_get_name(objEquipments_window))
				net_client_send(_CODE_CHANGE_ACTIVE_BOX, string(global.held_box.item.type)+"|"+string(mouseOn_box_i)+"|"+string(mouseOn_box_j)+"|"+box_get_confirmation_number_COMMON(mouseOn_box), BUFFER_TYPE_STRING)
				...
				net_client_send(_CODE_BUY, string(global.held_box.item.type)+"|"+string(global.held_box_i)+"|"+string(global.held_box_j)+"|"+box_get_confirmation_number_COMMON(global.held_box)+"|"+string(global.held_from.owner.npcID)+"|"+string(mouseOn_box_i)+"|"+string(mouseOn_box_j), BUFFER_TYPE_STRING)
		}
	}
}
